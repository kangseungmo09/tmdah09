...
with tab3:
    ...
    for ec in ec_values:
        avg_biomass[ec] = []
        for school_name, df in growth_data.items():
            avg_biomass[ec].append(df[df["EC"] == ec]["생중량"].mean())
    
    # 생중량 막대그래프
    for i, ec in enumerate(ec_values):
        growth_fig.add_trace(
            px.bar(x=list(growth_data.keys()), y=avg_biomass[ec], title=f"EC {ec} 평균 생중량").data[0],
